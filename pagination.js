...
request.onload = function(){
    ...
    function pagination(queryset,page,rows){
        ...
        let pages = Math.ceil(tabledata.length/rows) //100%5=20
        return {
            "queryset":trimedData,
            "pages" : pages
        }
    ...
    function pageButtons(pages){
        ...
        wrapper.innerHTML = ""
        let maxLeft = (state.page - Math.floor(state.window /2)); //-1
        let maxRight = (state.page + Math.floor(state.window /2)); // 3
        if(maxLeft<1){
            maxLeft = 1
            maxRight = state.window //5
        }
        if(maxRight > pages){
            maxLeft = pages-(state.window-1) // 20-(5-1)=16
            maxRight = pages //20
